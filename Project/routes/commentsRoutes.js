...
async function commentsRoutes(fastify, options) {
  ...
  fastify.get("/offers/:offerId/comments", async (request, reply) => {
    try {
  ...
  fastify.post("/offers/:offerId/comments", async (request, reply) => {
    try {
      const { offerId } = request.params;
  ...
  // **DELETE /offers/:offerId/comments** → Alle Kommentare eines Angebots löschen
  fastify.delete("/offers/:offerId/comments", async (request, reply) => {
    try {
      const { offerId } = request.params;
      ...
      if (filteredComments.length === comments.length) {
        return reply
          .code(404)
          .send({ message: "No comments found for this offer" });
      }

      await writeCommentsFile(filteredComments);

      reply.code(200).send({
        message: "All comments for the offer successfully deleted",
      });
    } catch (err) {
      fastify.log.error(err);
      reply.code(500).send({ message: "Error deleting comments" });
    }
  });
}
