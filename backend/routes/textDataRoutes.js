...
/**
 *  * KI-generiert von ChatGPT O-3 mini high: bis Zeile 45. Prompt: „Zeige mit einen Node.js-Codeausschnitt, der einen Promise-Wrapper für  SQLite-Methoden einbaut. Folgende Funktionen sollen vorhanden sein:
- run, get und all
...
async function fileRoutes(fastify, options) {
  ...
  fastify.get(
    "/offers/:offerId/files/:fileId/tags",
    { preHandler: authorize(["Account-Manager", "Developer", "User"]) },
    async (request, reply) => {
  ...
  fastify.put(
    "/offers/:offerId/files/:fileId/tags/:tagId",
    { preHandler: authorize(["Account-Manager", "User"]) },
    async (request, reply) => {
